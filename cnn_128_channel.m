...
for i=1:4
    for j=1:8
        layout_array(i,j)=32-(i-1)*8-(j-1);
        loc1(i,j)=i;
        loc2(i,j)=j;
    end
end
layout=[[layout_array;32+layout_array],[64+layout_array;96+layout_array]];
...
for session=1:2 % index of experiment sessions
    for i=1:20 % index of subjects
        ...
        for z = 1:length(data)
            newdata = zeros(2048,128);
            curr_array = data{z};
            ind = 1;
            for x = 1:16:256
                for y = x:(x+7)
                    newdata(:,ind) = ((curr_array(:,y+8)) + curr_array(:,y+8))/2;
                    ind = ind + 1;
                end
            end
            data{z} = newdata;
        ...
        for j=1:length(data)
            for t=floor(sig_start*fs_emg):floor(step_len*fs_emg):length(data{1,j})-floor(window_len*fs_emg)
                ...
                [sig_stft,f,t]=stft(sig,fs_emg,'Window',win,'OverlapLength',64,'FrequencyRange','onesided');                               
                sig_tensor=reshape(abs(sig_stft(1:32,:,reshape(layout,[1,128]))),[64,32,14]); 
...
layers = [imageInputLayer([32,14,64])
