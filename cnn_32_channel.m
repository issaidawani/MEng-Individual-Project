...
for i=1:1
    for j=1:8
        layout_array(i,j)=8-(i-1)*8-(j-1);
        loc1(i,j)=i;
        loc2(i,j)=j;
    end
end
layout=[[layout_array;8+layout_array],[16+layout_array;24+layout_array]];
...
for session=1:2 % index of experiment sessions
    for i=1:20 % index of subjects
        ...
        for z = 1:length(data)
            newdata = zeros(2048,32);
            curr_array = data{z};
            ind = 1;
            for x = 1:64:256
                for y = x:(x+7)
                    newdata(:,ind) = (curr_array(:,y) + curr_array(:,y+8) + ...
                        curr_array(:,y+16) + curr_array(:,y+24) + curr_array(:,y+32) + ...
                        curr_array(:,y+40) + curr_array(:,y+48) + curr_array(:,y+56))/8;
                    ind = ind + 1;
%                 for x=1:8
%                     newdata(:,x) = newdata(:,x) + curr_array(:,x+8);
%                     newdata(:,x+8) = newdata(:,x+8) + curr_array(:,x+16);
%                     newdata(:,x+16) = newdata(:,x+8) + curr_array(:,x+136);
%                     newdata(:,x+24) = newdata(:,x+8) + curr_array(:,x+144);
%                 end
            end
            data{z} = newdata;
            end
        ...
        toRemove = [33,32,31,28,27,24,19,18,13,10,5,4,1];
        ...
        for j=1:length(data)
            for t=floor(sig_start*fs_emg):floor(step_len*fs_emg):length(data{1,j})-floor(window_len*fs_emg)
                ...
                [sig_stft,f,t]=stft(sig,fs_emg,'Window',win,'OverlapLength',16,'FrequencyRange','onesided');                               
                sig_tensor=reshape(abs(sig_stft(1:8,:,reshape(layout,[1,32]))),[31,8,32]); 
...
layers = [imageInputLayer([8,32,31])
